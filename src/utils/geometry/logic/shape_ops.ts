...
import { rotate, translate, rotate_vec_90CW } from "./transform";
...
export function shape_contains<T extends AnyShape>(s1: T, s2: AnyShape): boolean {
    ...
    if (s1.name == ShapeNames.Circle) {
        ...
        if (s2.name == ShapeNames.Arc) {
            ...
            return (
                circle_intersect_count(circle, shape) === 0 &&
...
export function split_at_length<T extends AnyShape>(shape: T, at_length: number): (T | I_NULL_VALUE)[] {
    ...
    if (shape.name == ShapeNames.Segment) {
        ...
        const point_result = point_at_length(s, at_length);
        return [Segment(s.start, point_result as IPoint) as T, Segment(point_result as IPoint, s.end) as T];
    }
...
function point2point(a: IPoint, b: IPoint): [number, ISegment] {
    const dx = b.x - a.x;
    const dy = b.y - a.y;
    return [Math.sqrt(dx * dx + dy * dy), Segment(clone(a), clone(b))];
}

function point2segment(pt: IPoint, segment: ISegment): [number, ISegment] {
    ...
    const VEC_C = Vector(pt.x - segment.end.x, pt.y - segment.end.y);
    ...
    if (GE(start_sp, 0) && GE(end_sp, 0)) {
        ...
        return point2point(pt, segment.end);
    }
}

// Упрощённая версия для shape_contains - только подсчёт пересечений
function circle_intersect_count(circle: ICircle, shape: IArc | ICircle): number {
    // Простая проверка для shape_contains
    const { intersectArc2Circle, intersectCircle2Circle } = require('./intersect');
    if (shape.name === ShapeNames.Arc) {
        return intersectArc2Circle(shape as IArc, circle).length;
    }
    return intersectCircle2Circle(shape as ICircle, circle).length;
}
