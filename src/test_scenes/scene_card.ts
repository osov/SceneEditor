...
export async function run_scene_card() {
    ...
    EventBus.on('SYS_INPUT_POINTER_DOWN', (e) => {
        ...
        if (intersect_list.length > 0) {
            ...
            world_offset.set(wp.x - intersect_list[0].position.x, wp.y - intersect_list[0].position.y, 0);
        }
        log(intersect_list);
    });
    ...
    EventBus.on('SYS_INPUT_POINTER_MOVE', (e) => {
        ...
        if (intersect_list.length > 0) {
            ...
            intersect_list[0].set_position(pos.x - world_offset.x, pos.y - world_offset.y);
        }
    });
