...
export function sound_module() {
    function play(
        ...
        Services.audio_manager.set_volume(id, properties.gain);
        Services.audio_manager.set_pan(id, properties.pan);
        Services.audio_manager.set_speed(id, properties.speed);

        Services.audio_manager.set_end_callback(id, (type: SoundEndCallbackType) => {
        ...
        if (properties.delay > 0) {
            setTimeout(() => {
                Services.audio_manager.play(id, false, properties.gain, properties.speed, properties.pan);
    ...
    function stop(url: string | hash): void {
        const id = uh_to_id(url);
        Services.audio_manager.stop(id);
    }

    function pause(url: string | hash, pause: boolean): void {
        const id = uh_to_id(url);
        if (pause) {
            Services.audio_manager.pause(id);
        } else {
            Services.audio_manager.play(id, false, Services.audio_manager.get_volume(id), Services.audio_manager.get_speed(id), Services.audio_manager.get_pan(id));
        }
    }

    function set_gain(url: string | hash, gain: number): void {
        const id = uh_to_id(url);
        Services.audio_manager.set_volume(id, gain);
    }

    function set_pan(url: string | hash, pan: number): void {
        const id = uh_to_id(url);
        Services.audio_manager.set_pan(id, pan);
    }
