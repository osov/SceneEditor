...
function test() {
    ...
    main.list.forEach((node) => {
        ...
        let atlas = (node.data as any).atlas.replace('/assets/', '').replace('.atlas', '');
            ...
            case IObjectTypes.GO_SPRITE_COMPONENT:
                nodes.list.push({
                    ...
                    data: {
                        ...
                        atlas: `/assets/${mesh.atlas != '' ? mesh.atlas : 'default'}.atlas`,
    ...
    for (const file of data) {
        switch (file.type) {
            case DefoldType.COLLECTION:
                fs.writeFile(`${path_to_project}/${file.name}.collection`, file.data, (err: NodeJS.ErrnoException | null) => {
