...
const project_required_commands = [
    LOAD_PROJECT_CMD, 
    COPY_CMD, 
    RENAME_CMD, 
    DELETE_CMD, 
...
export async function handle_command<T extends CommandId>(cmd_id: T, params: object) {
    log('params: ', params)
    ...
    if (project_required_commands.includes(cmd_id)) {
        const resp = await _project_required(params);
        if (resp) return resp;
    }
...
async function _project_required(params: any) {
    const project_name = params[PROJECT_PARAM_NAME];
    if (!project_name)
...
async function on_save_data(cmd: ServerCommands[typeof SAVE_DATA_CMD]): Promise<ServerResponses[typeof SAVE_DATA_CMD]> {
    const data_path = get_data_file_path(cmd.project, cmd.path);
...
async function on_new_folder(cmd: ServerCommands[typeof NEW_FOLDER_CMD]): Promise<ServerResponses[typeof NEW_FOLDER_CMD]> {
    const folder_path = get_asset_path(cmd.project, path.join(cmd.path, cmd.name));
    ...
    const data = {path: path.relative(get_assets_folder_path(cmd.project), folder_path).split("\\").join("/")}
    return {result: 1, data};
}

async function on_get_folder(cmd: ServerCommands[typeof GET_FOLDER_CMD]): Promise<ServerResponses[typeof GET_FOLDER_CMD]> {
    const folder_path = get_asset_path(cmd.project, cmd.path);
    const dir_exists = await check_dir_exists(folder_path);
    if (!dir_exists)
        return {message: `${ERROR_TEXT.DIR_NOT_EXIST}: ${folder_path}`, result: 0};
    const root_folder = get_assets_folder_path(cmd.project);
...
async function on_rename(cmd: ServerCommands[typeof RENAME_CMD]): Promise<ServerResponses[typeof RENAME_CMD]> {
    const old_path = get_asset_path(cmd.project, cmd.path);
...
async function on_copy(cmd: ServerCommands[typeof COPY_CMD]): Promise<ServerResponses[typeof COPY_CMD]> {
    ...
    if (!file_exists)
        return {message: `${ERROR_TEXT.FILE_NOT_EXISTS}: ${old_path}`, result: 0};
    const new_path = get_asset_path(cmd.project, cmd.new_path);
...
async function on_delete(cmd: ServerCommands[typeof DELETE_CMD]): Promise<ServerResponses[typeof DELETE_CMD]> {
    const asset_path = get_asset_path(cmd.project, cmd.path);
...
async function on_save_info(cmd: ServerCommands[typeof SAVE_INFO_CMD]): Promise<ServerResponses[typeof SAVE_INFO_CMD]> {
    const current = await read_metadata(cmd.project, cmd.path);
    const updated = {...current, ...cmd.data};
    write_metadata(cmd.project, cmd.path, updated);
    return {result: 1};
}

async function on_get_info(cmd: ServerCommands[typeof GET_INFO_CMD]): Promise<ServerResponses[typeof GET_INFO_CMD]> {
    const data = await read_metadata(cmd.project, cmd.path);
    return {result: 1, data};
}
...
function check_fields(cmd_id: CommandId, params: any) {
    ...
    if (cmd_id === LOAD_PROJECT_CMD)
        wrong_fields = _check_fields(params, [PROJECT_PARAM_NAME]);
    if (cmd_id === GET_DATA_CMD)
        wrong_fields = _check_fields(params, [PROJECT_PARAM_NAME, PATH_PARAM_NAME]);
    if (cmd_id === GET_INFO_CMD)
        wrong_fields = _check_fields(params, [PROJECT_PARAM_NAME, PATH_PARAM_NAME]);
    if (cmd_id === SAVE_DATA_CMD)
        wrong_fields = _check_fields(params, [PROJECT_PARAM_NAME, PATH_PARAM_NAME, DATA_PARAM_NAME]);
    if (cmd_id === SAVE_INFO_CMD)
        wrong_fields = _check_fields(params, [PROJECT_PARAM_NAME, PATH_PARAM_NAME, DATA_PARAM_NAME]);
    if (cmd_id === GET_FOLDER_CMD)
        wrong_fields = _check_fields(params, [PROJECT_PARAM_NAME, PATH_PARAM_NAME]);
    if (cmd_id === NEW_FOLDER_CMD)
        wrong_fields = _check_fields(params, [PROJECT_PARAM_NAME, PATH_PARAM_NAME, NAME_PARAM_NAME]);
    if (cmd_id === RENAME_CMD)
        wrong_fields = _check_fields(params, [PROJECT_PARAM_NAME, PATH_PARAM_NAME, NEW_NAME_PARAM_NAME]);
    if (cmd_id === COPY_CMD)
        wrong_fields = _check_fields(params, [PROJECT_PARAM_NAME, PATH_PARAM_NAME, NEW_PATH_PARAM_NAME]);
    if (cmd_id === DELETE_CMD)
        wrong_fields = _check_fields(params, [PROJECT_PARAM_NAME, PATH_PARAM_NAME]);
