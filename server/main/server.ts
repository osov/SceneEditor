    ...
    router.add("GET", `${URL_PATHS.ASSETS}/*`, async (request, params) => {
        ...
        if (pathParts.length >= 1) {
            ...
            if (project) {
                ...
                if (file) {
                    return do_response(file, false, undefined, request, !(filePath.includes('.json') || filePath.includes('.tilesinfo')));
                }
            }
        ...
        if (session?.project) {
            ...
            if (file) {
                return do_response(file, false, undefined, request, !(asset_path.includes('.json') || asset_path.includes('.tilesinfo')));
            }
        }
        return do_response("404 Not Found", false, 404, request);
    });
